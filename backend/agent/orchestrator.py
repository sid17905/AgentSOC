...
async def run_agent(
    ...
    if OpenAIClient is None:
        fallback_reason = "OpenAI-compatible SDK package is not installed."
        log_thought(incident, f"[Groq] {fallback_reason}")
        ...
        while state.should_continue and state.iteration < state.max_iterations:
            ...
            try:
                response = await asyncio.to_thread(_call_groq, prompt)
                parsed = _extract_json_object(_model_content(response).strip())
            except Exception as exc:
                fallback_reason = f"Groq or JSON parsing failed: {exc}"
                log_thought(incident, f"[Groq] {fallback_reason}")
                break
